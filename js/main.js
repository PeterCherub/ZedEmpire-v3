...
function newGame(inheritedBankSavings) {
...
// ============================================================
//  PLAYER NAME (stored locally, sent with every leaderboard entry)
// ============================================================
const PLAYER_NAME_KEY = "zedempire-player-name";
...
const playerNameOverlay = document.getElementById("playername-overlay");
const playerNameInput = document.getElementById("playername-input");
const playerNameStatus = document.getElementById("playername-status");
const playerNameSaveBtn = document.getElementById("playername-save");
const playerNameCancelBtn = document.getElementById("playername-cancel");

function openPlayerName() {
  playerNameStatus.textContent = "";
  playerNameStatus.className = "";
  playerNameInput.value = getPlayerName();
  playerNameOverlay.classList.remove("hidden");
  playerNameInput.focus();
}

function closePlayerName() {
  playerNameOverlay.classList.add("hidden");
}

playerNameCancelBtn.addEventListener("click", closePlayerName);

playerNameSaveBtn.addEventListener("click", () => {
  const name = playerNameInput.value.trim();
  setPlayerName(name);
  playerNameStatus.textContent = name ? "Saved." : "Cleared -- runs will show as Anonymous.";
  playerNameStatus.className = "success";
  setTimeout(closePlayerName, 700);
});
...
function openFeedback() {
  ...
  feedbackName.value = getPlayerName();
...
function onPointerDown(x, y) {
  ...
  if (settingsOpen) {
    ...
    else if (action === "playerName") { settingsOpen = false; openPlayerName(); }
  ...
  if (gameState === "gameOver") {
    if (gameOverRetryBtn.click(x, y)) { audio.play("click"); newGame(bank.savings); return; }
...
function triggerLoss(reason) {
  ...
  lossCount += 1;
  const isWipe = lossCount % 3 === 0; // every 3rd loss, not every loss from the 2nd on
  const outcome = isWipe ? "robbed" : "setback";
  if (isWipe) {
    const lostSavings = bank ? bank.savings : 0;
    if (bank) bank.savings = 0;
    gameOverText = `LOSS #${lossCount}: ${reason} While you were trying to recover, robbers emptied your bank account${lostSavings > 0 ? ` (K${lostSavings} lost)` : ""}. You survive, but you must start again with nothing.`;
  } else {
    const untilWipe = 3 - (lossCount % 3);
    gameOverText = `SETBACK #${lossCount}: ${reason} You survived, and your bank savings are untouched. ${untilWipe} more loss${untilWipe === 1 ? "" : "es"} and robbers will empty your account.`;
  }
  recordRun({
    chapter: player.chapter,
    province: getChapter(player.chapter).province,
...
function render() {
  ...
  if (gameState === "playing" || gameState === "map" || gameState === "consequence") {
    // The map screen's panel starts lower than the playing/consequence
    // ledger does, so the button needs a different safe spot on each.
    if (gameState === "map") pauseBtn.setPosition(W - 58, 34);
    else pauseBtn.setPosition(W - 58, 122);
    pauseBtn.draw(ctx);
  }
...
function drawGameOver() {
  draw.centeredTitle(ctx, lossCount % 3 === 0 ? "ROBBED — START AGAIN" : "SETBACK — RECOVER", 220, W);
  ctx.fillStyle = `rgb(${theme.creamDim[0]*255}, ${theme.creamDim[1]*255}, ${theme.creamDim[2]*255})`;
  ctx.font = fonts.get(14);
  ctx.textAlign = "center";
  ctx.textBaseline = "top";
  ...
  ctx.fillStyle = `rgb(${theme.gold[0]*255}, ${theme.gold[1]*255}, ${theme.gold[2]*255})`;
  ctx.fillText(lossCount % 3 === 0 ? "Bank emptied after the robbery." : "No money is saved from losing.", W / 2, noteY, W - 80);
