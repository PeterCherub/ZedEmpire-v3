...
// entry: { chapter, province, day, outcome ("setback"|"robbed"|"completed"),
//          stats: {...}, headline, timestamp }
export function recordRun(entry) {
  const runs = getRuns();
  runs.push({ ...entry, timestamp: Date.now() });
  runs.sort((a, b) => (b.chapter - a.chapter) || (b.day - a.day) || (b.timestamp - a.timestamp));
